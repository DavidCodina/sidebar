...
class LinkFx {
  constructor(el){
    this.DOM                = {el: el};
    this.options            = { animation: { text: false, line: true }};
  ...
  createTimeline(){
    ...
    this.tl = gsap.timeline({
      paused: true,
      onStart: () => {
        if (this.options.animation.line){ this.DOM.line.style.filter = `url(${this.filterId}`; }
        if (this.options.animation.text){ this.DOM.text.style.filter = `url(${this.filterId}`; }
      },

      onComplete: () => {
        if (this.options.animation.line){ this.DOM.line.style.filter = 'none'; }
        if (this.options.animation.text){ this.DOM.text.style.filter = 'none'; }
      }
    });
  }
}
...
class Pulse extends LinkFx {
  constructor(el){
    super(el);
    this.filterId = '#filter-pulse';
